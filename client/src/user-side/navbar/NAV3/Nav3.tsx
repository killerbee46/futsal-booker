import { Affix, Col, Divider, Drawer, Flex, Image, Row, Space } from "antd";
...
export const Nav3 = ({
  ...
  return (
    <Affix>
      <MainContainer
        wraperClassName="py-[12px]"
        wraperStyle={{
          background: backgroundColor,
        }}
      >
        <Row justify={"center"} className="hidden md:block">
          <Col flex={1} className="justify-center">
            {logo && (
              <Link
                href="./"
                className="flex justify-center items-center gap-4"
              >
                <Image preview={false} src={logo} height={60} alt="LOGO" />
                <Text className="uppercase text-xl !font-bold">
            ...
            {menus?.map((d: MenuItemTypes, i: number) => {
              return (
                <Link key={i} href={d?.path}>
                  <Text className=" py-[6px] hover:border-b-[2px] hover:border-primary text-[16px] font-medium">
                    {d?.label}
                  </Text>
                </Link>
              );
